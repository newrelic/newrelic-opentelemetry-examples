...
function instrumentTimerFunction(originalFunction) {
  const tracer = opentelemetry.trace.getTracer('azure-timer-function-tracer');

  return wrappedTimerFunction;

  async function wrappedTimerFunction(context, myTimer) {
    ...
    const span = tracer.startSpan(
      executionContext.functionName,
      {
        ...
        attributes: {
          ...
          [SemanticAttributes.FAAS_TRIGGER]: FaasTriggerValues.TIMER,
          [SemanticAttributes.FAAS_EXECUTION]: executionContext.invocationId,
          [SemanticAttributes.FAAS_COLDSTART]: _isColdStart(),
          [SemanticAttributes.FAAS_TIME]: new Date(bindingData.timerTrigger).toISOString(),

          // Interesting information that may be useful but is not required by New Relic or Open Telemetry.
          'faas.azure.isPastDue': myTimer.isPastDue,
          'faas.azure.next': myTimer.scheduleStatus.next
        }
      }
    );
